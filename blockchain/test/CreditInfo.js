...
describe("CreditInfo", () => {
  ...
  it("Is adding credit Info", async () => {
    ...
    await tx1.wait();
    const [, , , , value] = await creditInfo
      .connect(user)
      .accessCreditInfo(user.address);
  ...
  describe("Registered User", async () => {
    ...
    it("Access Info by permission", async () => {
        /*Accessing as owner */
        let info = await creditInfo
        .connect(owner)
        .accessCreditInfo_Owner(user.address);
        expect(info);
    ...
    it("Is Updating payment history", async () => {
        const tx = await creditInfo.connect(owner).updatePaymentHistory(user.address,5);
        await tx.wait();
        expect(await creditInfo.paymentHistory(user.address)).to.not.equal(0);
    });
    
    it("Is calculating credit score", async () => {
        const tx1 = await creditInfo.connect(owner).updatePaymentHistory(user.address,5);
        await tx1.wait();
        
        const tx = await creditInfo
        .connect(owner)
        .calculateCreditScore(user.address);
        await tx.wait();
        
        
        const person = await creditInfo.connect(user).accessCreditInfo(user.address);
        expect(person.creditScore).to.not.equal(0);
    });

    it("updates Credit Info by owner", async()=>{
        const  tx2  = await creditInfo.connect(owner).updateCreditInfo_Owner(user.address, 200000,10000, 1);
         await tx2.wait();
        const person = await creditInfo.connect(user).accessCreditInfo(user.address);
        console.log(person);
    });
