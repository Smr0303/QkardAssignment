...
function UserDashboard({ account, provider, CreditContract }) {
  ...
  const accessCreditInfo = async () => {
    if (account) {
      ...
      const authorisationStatus = await CreditContract.connect(
        signer
  ...
  const handleSubmit = async (e) => {
    ...
    toast.success("Registering.. Wait few minutes");
  ...
  const handleUpdate = async (e) => {
    ...
    if (selectedOption === type) {
      ...
      const signer = await provider.getSigner();
      const typeValue = selectedOption == "Individual" ? 0 : 1;
      const tx = await CreditContract.connect(signer).updateCreditInfo(
        typeValue
      );
      toast.success("Updating...Please remain standby");
      const receipt = await tx.wait();
      if (receipt) {
    ...
    <div>
      <Form type="submit">
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          <Form.Control
            type="input"
            value={account ? account : "Your Address"}
            placeholder=""
            disabled
          />
        </Form.Group>
